...
import os
import numpy as np
from py_3d_construct_lib.face_point_cloud import face_point_cloud
from py_3d_construct_lib.partitionable_spheroid_triangle_mesh import PartitionableSpheroidTriangleMesh
from py_3d_construct_lib.mesh_utils import write_shell_maps_to_stl
...
def create_face_mesh_stl(face_key="m", output_dir="output"):
    ...
    mesh = PartitionableSpheroidTriangleMesh.from_point_cloud(points, vertex_labels=labels)
    ...
    print(f"Original dimensions: {dimensions[0]:.4f} x {dimensions[1]:.4f} x {dimensions[2]:.4f} units")
    ...
    print(f"Scaling by factor {scale_factor:.2f} to make longest side {target_size_mm}mm")
    ...
    print(f"Scaled dimensions: {dimensions_scaled[0]:.1f} x {dimensions_scaled[1]:.1f} x {dimensions_scaled[2]:.1f} mm")
    ...
    print(f"Perforated mesh has {len(perforated_mesh.vertices)} vertices and {len(perforated_mesh.faces)} faces")
    ...
    front_face_count = sum(1 for region_id in face_to_region_map.values() if region_id == 0)
    back_face_count = sum(1 for region_id in face_to_region_map.values() if region_id == 1)
    ...
    front_region = TransformedRegionView(
        partition=partition,
        region_id=0,
        transform=np.eye(4)  # Identity transform
    )
...
def main():
    ...
    try:
        ...
        mesh_stats = PartitionableSpheroidTriangleMesh.from_point_cloud(points, vertex_labels=labels)
        ...
        print(f"   • Triangulated mesh: {len(mesh_stats.vertices)} vertices, {len(mesh_stats.faces)} faces")
        print(f"   • Final dimensions: {dimensions_final[0]:.1f} x {dimensions_final[1]:.1f} x {dimensions_final[2]:.1f} mm")
