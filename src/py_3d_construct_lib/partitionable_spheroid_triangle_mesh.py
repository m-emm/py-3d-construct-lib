...
class PartitionableSpheroidTriangleMesh:
    ...
    #         raise ValueError(
    #             "Barycentric coordinates must sum to 1.0, got: "
    #             f"{barycentric_coords} (sum={sum(barycentric_coords)})"
    #         )

    #     if any(coord < 0 for coord in barycentric_coords):
    #         raise ValueError(
    #             "Barycentric coordinates must be non-negative, got: "
    #             f"{barycentric_coords}"
    #         )

    #     # check if more than one coordinate is zero
    #     if len([coord for coord in barycentric_coords if coord < 1e-6]) > 1:
    #         raise ValueError(
    #             "Barycentric coordinates must have at most one zero component (because otherwise, they represent just one vertex), got: "
    #             f"{barycentric_coords}"
    #         )
    ...
    #     # check if the the new vertex is on the edge of the face
    #     if any([coord < 1e-6 for coord in barycentric_coords]):
    #         # find the edge where the new vertex is located

    #         edge = None
    #         for i in range(3):
    #             if barycentric_coords[i] < 1e-6:
    #                 # this means the new vertex is on the edge which does not contain this vertex
    #                 edge = normalize_edge(*[face[j] for j in range(3) if j != i])
    #                 break

    #         if edge is None:
    #             raise ValueError(
    #                 "Could not find the edge where the new vertex is located. "
    #                 "This should not happen if the barycentric coordinates are valid."
    #             )

    #         new_labels.append(
    #             self.vertex_labels[edge[0]] + "_" + self.vertex_labels[edge[1]]
    #         )

    #         edge_to_tri = calc_edge_to_triangle_map(self.faces)

    #         # find the triangle across the edge

    #         edge_triangle_indices = edge_to_tri.get(edge, [])

    #         new_faces = [
    #             tri for i, tri in enumerate(new_faces) if i not in edge_triangle_indices
    #         ]

    #         if len(edge_triangle_indices) != 2:
    #             raise ValueError(
    #                 f"Expected exactly 2 triangles for edge {edge}, but found {len(edge_triangle_indices)}."
    #             )

    #         # Get the two triangles
    #         tri1 = self.faces[edge_triangle_indices[0]]
    #         tri2 = self.faces[edge_triangle_indices[1]]

    #         # Ensure we know which vertices are which
    #         a, b = edge

    #         # Find the third vertex (not a or b) in each triangle
    #         c = next(v for v in tri1 if v not in edge)
    #         d = next(v for v in tri2 if v not in edge)
    ...
    #         # Append them to new_faces
    #         new_faces.extend(new_triangles)

    #     else:
    #         # Inside triangle: replace 1 triangle with 3 new ones
    #         new_labels.append("_".join(self.vertex_labels[i] for i in face))

    #         # Remove the original face
    #         new_faces.pop(face_index)

    #         # Add 3 new triangles connecting the new vertex to the original triangle's edges
    #         i0, i1, i2 = face
    #         new_triangles = [
    #             [i0, i1, new_index],
    #             [i1, i2, new_index],
    ...
    # def perforate_along_plane(self, plane_point, plane_normal, epsilon=1e-8):
    #     """
    #     Inserts new vertices along the edges of the mesh that are intersected by a plane.
    #     Uses a geometric formulation with no sign fiddling:
    #     compute t from plane intersection, insert if 0 < t < 1.

    #     Parameters:
    #     - plane_point: a point on the cutting plane
    #     - plane_normal: the normal vector of the cutting plane
    #     - epsilon: tolerance for detecting near-zero denominators

    #     Returns:
    #     - A new mesh with added vertices along the cut plane
    #     """
    #     new_mesh = self
    #     seen_edges = set()
    ...
    #             a_idx = face[i]
    #             b_idx = face[(i + 1) % 3]
    #             edge = normalize_edge(a_idx, b_idx)

    #             if edge in seen_edges:
    #                 continue
    #             seen_edges.add(edge)

    #             va = self.vertices[a_idx]
    #             vb = self.vertices[b_idx]
    ...
    #                 continue  # Edge is parallel to plane, no intersection

    #             t = -np.dot(plane_normal, w) / denom

    #             if 0 < t < 1:
    #                 # Compute barycentric coordinates for face: one vertex gets 0
    #                 bary = [0.0, 0.0, 0.0]
    #                 if i == 0:
    #                     bary[1] = 1 - t
    #                     bary[2] = t
    ...
    #                 _logger.info(
    #                     f"Adding vertex in face {face_idx} with barycentric coords {bary}, vertices are {new_mesh.vertex_labels[a_idx]}, {new_mesh.vertex_labels[b_idx]}"
    #                 )
    #                 new_mesh = new_mesh.add_vertex_in_face(face_idx, bary)
    ...
    def _perforate_along_plane_bulk(self, plane_point, plane_normal, epsilon):
        ...
        def check_directed_edge_duplicates(faces):
            directed_edges = Counter(
                (a, b) for f in faces for a, b in zip(f, f[1:] + f[:1])
            )
            for (a, b), count in directed_edges.items():
                if count > 1:
                    _logger.info(
                        f"🔁 Duplicated directed edge: ({a} → {b}) appears {count} times"
                    )

            canonical_edges = Counter(
                tuple(sorted((a, b))) for f in faces for a, b in zip(f, f[1:] + f[:1])
            )
            for e, count in canonical_edges.items():
                if count != 2:
                    _logger.info(f"❌ Edge {e} appears {count} times")
        ...
        def orig_sign(tri_idxs):
            key = tuple(tri_idxs)
            if key not in orig_area_sign_cache:
                a, b, c = tri_idxs
                orig_area_sign_cache[key] = np.dot(
                    np.cross(V_new[b] - V_new[a], V_new[c] - V_new[a]), plane_normal
                )
            return orig_area_sign_cache[key]
        ...
        for tri_index, tri in enumerate(F_orig):
            ...
            k = len(cuts)
            _logger.info(f"Cuts: {cuts} k={k} for triangle {tri}")
            ...
            if k == 0:
                ...
                p, q, newIdx = cuts[0]
                r = 3 - (p + q)  # the remaining index in {0,1,2}

                A = tri[r]
                B = tri[p]
                C = tri[q]

                # Candidate sub-triangles:
                cand1 = [A, B, newIdx]
                cand2 = [A, newIdx, C]

                if orig_sign(cand1) * orig_sign(tri) > 0:
                    F_new.append(cand1)
                else:
                    F_new.append(cand1[::-1])
